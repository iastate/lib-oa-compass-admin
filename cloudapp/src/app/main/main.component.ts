...
import {
  ...
  REQUIRED_ROLE_NAMES,
} from '../services/operator-authorization.service';
...
export class MainComponent implements OnInit {
  ...
  readonly requiredOperatorRoles = REQUIRED_ROLE_NAMES.join(', ');
  private entityWatchStarted = false;
  ...
  constructor(
    ...
    private operatorAuthorization: OperatorAuthorizationService,
  ) {
  ...
  private async initializeAuthorization(): Promise<void> {
    ...
    this.actionStatus = this.translate.instant('oa.auth.checking');

    const result = await this.operatorAuthorization.ensureAuthorized();

    this.authorizationLoading = false;
    this.isAuthorized = result.authorized;
    this.authorizationError = result.error || undefined;
    ...
    if (!result.authorized) {
      ...
      this.actionStatus = result.error
        ? this.translate.instant('oa.auth.lookupFailedStatus')
        : this.translate.instant('oa.auth.deniedStatus');
      return;
    }

    this.actionStatus = '';
    this.startEntityContextWatch();
  }

  private startEntityContextWatch(): void {
    if (this.entityWatchStarted) return;
    this.entityWatchStarted = true;
